...
#
        with open(j,'r') as f:
            ...
            print(InChI_key, index, nAtoms,A, B, C,mu,HOMO,LUMO,gap,ZPVE,E,E0K,H,G,tEsolvation,cV)
            abc = getMullikenCharge(f, nAtoms)
            print(abc) 
            outputData += ("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (nAtoms,index,mu,HOMO,LUMO,gap,ZPVE,E,E0K,H,G,tEsolvation,cV))
            outputData += "\n" + getElementType(f) + "\n"# + getFrequencies(f)
