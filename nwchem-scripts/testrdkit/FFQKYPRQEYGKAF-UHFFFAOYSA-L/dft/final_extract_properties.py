...
def getSMILES(inchi_key):
    with open('Yarrowia_Reactions.txt', 'r') as datafile:
         lines=datafile.readlines()
         for line in lines:
             if line.split('\t')[2].split('=')[1].replace('"', '')==inchi_key: 
                 smiles = line.split('\t')[3].replace('"', '') 
#
                 os.chdir(inchi_key)
                 xyz_files = []
...
#
                 ...
                 os.chdir('..')
                 return ('%s\t%s' %(smiles, converged_smiles))
...
#
                 ...
                 os.chdir('..')
                 return (('%s\t%s' %(inchi, converged_inchi)))
...
dirs = [line.rstrip('\n') for line in open('converged')]

print(dirs)

for each_dir in dirs:
    pattern = each_dir+'.out.6*'
...
#
        with open(j,'r') as f:
            ...
            nAtoms = int(getNumberOfAtoms(each_dir))
            ...
            SMILES = getSMILES(each_dir)
            InChI = getInChI(each_dir)
            print(each_dir)
            print('\n')
               
            os.chdir(each_dir)
            outputData = ("%s\n%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (nAtoms, each_dir, index, A, B, C, mu, HOMO, LUMO, gap, ZPVE, E0K, E, H, G, cV))
